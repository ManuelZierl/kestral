(function () {
  var PROTOCOL = "app-host-surface-bridge", VERSION = 3;
  ...
  function flush() {
    ...
    if (readyRequested) {
      ...
      send({ protocol: PROTOCOL, v: VERSION, type: "ready", instanceId: instanceId });
    }
    var queued = preInit;
    preInit = [];
    queued.forEach(function (q) { dispatchOp(q.op, q.resolve, q.reject, q.onProgress); });
    var states = preStates;
    preStates = [];
  ...
  function requestOp(op, onProgress) {
    return new Promise(function (resolve, reject) {
      if (instanceId === null) {
        preInit.push({ op: op, resolve: resolve, reject: reject, onProgress: onProgress });
        return;
      }
  ...
  function applyTheme(theme, variables) {
    ...
    if (window.appHost) {
      window.appHost.theme = theme;
      window.appHost.variables = ctx.variables;
    }
  }
  window.addEventListener("message", function (e) {
    ...
    if (m.type === "init") {
      ...
      ctx.appId = m.appId;
      ctx.surface = m.surface;
      ctx.capabilities = m.capabilities || [];
      ctx.configSchema = m.configSchema;
      ctx.config = m.config || {};
      ctx.extensionContext = m.extensionContext || {};
      ctx.hostContext = m.hostContext || {};
      applyTheme(m.theme, m.variables);
      var h = window.appHost;
      h.appId = ctx.appId;
      h.surface = ctx.surface;
      h.capabilities = ctx.capabilities;
      h.configSchema = ctx.configSchema;
      h.config = ctx.config;
      h.extensionContext = ctx.extensionContext;
      h.hostContext = ctx.hostContext;
      flush();
